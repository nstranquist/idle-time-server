...
module.exports = {
  ...
  addOne: async (req, res, next) => {
    ...
    if (!taskData) { return handleError(res, 'Invalid request, taskData is undefined'); }
  ...
  updateOrder: async (req, res, next) => {
    ...
    try {
      ...
      if (JSON.stringify(order) == JSON.stringify(oldOrder)) return res.status(400).json({ status: 'error', message: 'order is the same as before', data: null });
