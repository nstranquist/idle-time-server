...
module.exports = {
  ...
  addOne: async (req, res, next) => {
    ...
    try {
      ...
      return res.status(200).json({
        status: 'success', message: 'added your timelog', data: { timelog: updatedUser.timelogs[length] },
      });
    } catch (error) {
      console.log('error:', error);
      return res.status(400).json({
        status: 'error', message: "could not add the timelog to user's timelogs", data: null,
      });
    }
  },
  updateOne: async (req, res, next) => {
    ...
    try {
      const updatedUser = await user.save();
      console.log('updated user:', updatedUser);
      return res.status(200).json({ status: 'success', message: 'updated your timelog', data: { timelog: timelogId } });
    } catch (error) {
      console.log('error:', error);
      return res.status(400).json({ status: 'error', message: 'could not update your timelog', data: null });
    }
  },
  deleteOne: async (req, res, next) => {
    ...
    try {
      ...
      res.status(200).json({
        status: 'success', message: "deleted user's timelog successfully", data: { id: timelogId },
      });
    } catch (error) {
      console.log('error:', error);
      next();
