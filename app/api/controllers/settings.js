...
module.exports = {
  ...
  getSection: async (req, res, next) => {
    ...
    else console.log('user settings found:', userSettingsSection)

    if(settings)
      res.status(200).json({ status: "success", message: "found your settings!", data: { settingsSection: userSettingsSection } })
    else
  ...
  updateSection: async (req, res, next) => {
    const userId = req.body.userId;
    const sectionName = req.params.name;
    const sectionSettings = req.body.sectionSettings;
